...
class ImprimirConsolidadoControlMinsa
{
    public $anio;
    public $mes;
    public function imprimirFichaConsolidadoMinsa()
    {
        ...
        $detalleLicencias = ReportesControlador::ctrControlPersonalMinsa($mes, $anio);
        ...
        $pdf->SetTitle('Consolidado Control Minsa Covid- Legado y Residentes ' . $dataMes["descMes"] . '-' . $anio);
        ...
        // -----------------------------------------------------------------------------
        $htmlhead = '<h1 style="text-align:center;">RELACION DE PERSONAL RESIDENTE Y LEGADO MINSA CON LICENCIA DEL MES DE ' . $dataMes["descMes"] . '- ' . $anio . ' <br>DE LA COORDINACION DEL EQUIPO DE GESTION DEL DESARROLLO Y RH</h1>';
    ...
    EOD;
        ...
        $pdf->Output('Consolidado Licencias Legado Minsa,Covid y residentes ' . $dataMes["descMes"] . '-' . $anio . '.pdf', 'I');
    }
}
$fichaImprimir = new ImprimirConsolidadoControlMinsa();
$fichaImprimir->mes = $_GET["mes"];
$fichaImprimir->anio = $_GET["anio"];
$fichaImprimir->imprimirFichaConsolidadoMinsa();
