...
class ImprimirConsolidadoControlSubsidioMas20Anu
{
    public $anio;
    public function imprimirFichaConsolidadoSubsidiomas20Anu()
    {
        require_once "../util/tcpdf/headControl.php";
        $anio = $this->anio;

        // create new PDF document
        $detalleLicenciasSub20 = ReportesControlador::ctrControlPersonalSubsidioMas20Anual($anio);
        $pdf = new MYPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);

        // set document information
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor('OCP-HNSEB');
        $pdf->SetTitle('Consolidado Subsidio mayor de 20 días Año - ' . $anio);
        ...
        $pdf->AddPage('L', 'A4');
        // -----------------------------------------------------------------------------
        $htmlhead = '<h1 style="text-align:center;">RELACION DE PERSONAL QUE SUPERA LOS 20 DIAS DE LICENCIA - ' . $anio . ' <br>COORDINACION DEL EQUIPO DE GESTION DEL DESARROLLO Y RH</h1>';
        ...
        foreach ($detalleLicenciasSub20 as $key => $item) {
            ...
            $detalleSub20 = ReportesControlador::ctrSubsidioMas20AcumDifAnu($anio,$item["empleado"]);
        ...
        $pdf->Output('Consolidado Licencias Subsidio > 20 Año ' . $anio . '.pdf', 'I');
    }
}
$fichaImprimir = new ImprimirConsolidadoControlSubsidioMas20Anu();
$fichaImprimir->anio = $_GET["anio"];
$fichaImprimir->imprimirFichaConsolidadoSubsidiomas20Anu();
